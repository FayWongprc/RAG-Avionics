...
def evaluate_logic(
    ...
    """
    from .prompts import TEST_LOGIC_EVALUATION_PROMPT, REQUIREMENT_COVERAGE_EVALUATION_PROMPT
    ...
    # 创建原子需求字典，方便查找
    atomic_req_dict = {}
    for _, row in df_atomic.iterrows():
        req_id = row.get("atomic_req_id")
        if not pd.isna(req_id):
            atomic_req_dict[req_id] = {
                "atomic_req_id": req_id,
    ...
    if enable_llm:
        ...
        for idx, row in df_test_cases.iterrows():
            ...
            design_rationale = row.get("design_rationale", "")
            preconditions = row.get("preconditions", "")
            inputs = row.get("inputs", "")
            steps = row.get("steps", "")
            expected = row.get("expected_results", "")
            atomic_req_id = row.get("trace_to_atomic_req", "")
            
            # 获取原子需求信息
            atomic_req = atomic_req_dict.get(atomic_req_id, {})
            
            # 构建评估 prompt（包含原子需求信息）
            prompt = TEST_LOGIC_EVALUATION_PROMPT.format(
                atomic_req_id=atomic_req.get("atomic_req_id", "UNKNOWN"),
                atomic_req_category=atomic_req.get("category", "未知"),
                atomic_req_statement=atomic_req.get("statement", "未知"),
                tc_id=tc_id,
                test_method=test_method,
                objective=objective,
                design_rationale=design_rationale if design_rationale else "无",
        ...
        # 第三步：评估每个原子需求的测试覆盖完整性
        if llm is not None:
            print(f"\n🔍 开始评估原子需求的测试覆盖完整性 ({len(all_atomic_req_ids)} 个需求)...")
            
            for req_id in all_atomic_req_ids:
                atomic_req = atomic_req_dict.get(req_id, {})
                
                # 获取该需求的所有测试用例
                req_test_cases = df_test_cases[df_test_cases["trace_to_atomic_req"] == req_id]
                
                # 构建测试用例摘要
                test_cases_summary = ""
                if len(req_test_cases) == 0:
                    test_cases_summary = "⚠️ 当前没有任何测试用例覆盖此需求"
                else:
                    test_cases_summary = f"当前有 {len(req_test_cases)} 个测试用例:\n\n"
                    for idx, tc_row in req_test_cases.iterrows():
                        test_cases_summary += f"- **{tc_row.get('tc_id')}**: {tc_row.get('title')}\n"
                        test_cases_summary += f"  - 测试方法: {tc_row.get('test_method')}\n"
                        test_cases_summary += f"  - 测试目的: {tc_row.get('objective')}\n\n"
                
                # 构建评估 prompt
                prompt = REQUIREMENT_COVERAGE_EVALUATION_PROMPT.format(
                    atomic_req_id=atomic_req.get("atomic_req_id", "UNKNOWN"),
                    atomic_req_category=atomic_req.get("category", "未知"),
                    atomic_req_statement=atomic_req.get("statement", "未知"),
                    atomic_req_source_text=atomic_req.get("source_text", "无"),
                ...
                try:
                    ...
                    requirement_coverage_analysis.append({
                        "atomic_req_id": req_id,
                        "covered_methods": result.get("covered_methods", []),
                        "missing_required_methods": result.get("missing_required_methods", []),
                        "missing_conditional_methods": result.get("missing_conditional_methods", []),
                        "is_complete": result.get("is_complete", False),
                        "completeness_score": result.get("completeness_score", 0),
                        "suggestions": result.get("suggestions", [])
                    })
                    
                    print(f"  ✓ {req_id}: 完整性 {result.get('completeness_score', 0)}%")
                    
                except Exception as e:
                    print(f"  ✗ {req_id}: 评估失败 - {e}")
                    requirement_coverage_analysis.append({
                        "atomic_req_id": req_id,
                        "error": str(e)
                    })
    ...
    return LogicEvaluation(
        ...
        requirement_coverage_analysis=requirement_coverage_analysis,
    )
